...
5. **UX verification checklist** – Hover and press <kbd>Tab</kbd> to move focus into the history controls, then use <kbd>Arrow</kbd> keys / <kbd>Home</kbd> / <kbd>End</kbd> to cycle through commits. Confirm each point announces commit hash, localized timestamp, and KB size, that focus rings meet WCAG contrast requirements, the history tooltip panel below the chart updates accordingly, and that the “More history needed” banner appears when fewer than five commits are available.
